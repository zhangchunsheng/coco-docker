...
coco数据库基于nideshop中的[nideshop.sql](https://github.com/tumobi/nideshop/blob/master/nideshop.sql)数据库，
然后在实际开发过程中进行了调整和修改:

* 删除了一些目前不必要的表；
* 删除了表中一些目前不必要的字段；
* 行政区域数据coco_region没有采用原nideshop中的数据，而是采用了[Administrative-divisions-of-China](https://github.com/modood/Administrative-divisions-of-China)；
* 表中的某些字段采用JSON存储；
* 表中的日期或时间字段采用DATE、DATETIME；
* 字段的数据类型粗粒度化，例如避免MEDIUMINT，而是INT；
* 表的数据做了清理、调整和补充（假数据）。

coco数据库由三个sql文件组成，在coco-db文件夹下面的sql文件夹中：
...
综上，这里coco真正必须运行的sql文件是coco_table.sql，其他两个sql文件开发者自行决定如何是否使用。
...
这里商品存在商品表（coco_goods)，商品属性表（coco_goods_attribute），商品规格表（coco_goods_specification），商品货品表（coco_goods_product）四种表
...
目前准备支持用户普通账号登录和微信登录两种方式，两种登录方式仅仅采用一个coco-user表可能不是很合适。

外，如果进一步支持其他多种第三方登录，那么这里需要重新设计。

### 2.3 行政区域设计
 
coco_region表保存了行政区域信息，包括省级、市级、县级三个等级，

原nideship.sql中存在region数据，但是coco.sql的region数据则来自
...
由coco_order表和coco_order_goods表保存。
...
所对应的后台服务方法是coco-wx-api模块的`WxOrderController.submit`方法。
...
所对应的后台服务方法是coco-wx-api模块的`WxOrderController.prepay`
...
所对应的后台服务方法是coco-wx-api模块的`WxOrderController.cancel`
...
对应的应该是coco-admin-api模块的系统定时任务的`OrderJob.checkOrderUnpaid`
...
所对应的后台服务方法是coco-wx-api模块的`WxOrderController.payNotify`
...
所对应的后台服务方法是coco-wx-api模块的`WxOrderController.refund`
...
所对应的后台服务方法是coco-admin-api模块的`AdminOrderController.refundConfirm`
...
所对应的后台服务方法是coco-admin-api模块的`AdminOrderController.ship`

* 301 -> 401

当用户收到商品以后，用户点击`收货确认`按钮，设置订单状态401，表示用户成功收货。

所对应的后台服务方法是coco-wx-api模块的`WxOrderController.confirm`
...
应该改为 coco-admin-api模块的系统定时任务`OrderJob.checkOrderUnconfirm`
...
coco_order表中存在`aftersale_status`字段，记录订单售后状态。
而具体的售后记录则是coco_aftersale表记录。
...
这里需要补充的是：订单coco_order表的`aftersale_status`字段，和订单售后coco_aftersale
...
在coco_order表中存在`comments`字段，表示有几个订单商品没有评价；
而在coco_order_goods表中存在`comment`字段，表示当前订单商品的评论ID。
...
评论表coco_comment保存评论相关的信息，其中最关键的是`type`字段和`value_id`字段。
...
团购是由团购规则表coco_groupon_rules和团购活动表coco_groupon组成。

管理员在管理后台对一些商品配置团购规则，保存在coco_groupon_rules表中。

用户在小商场中则看到团购规则给出的优惠信息。
接下来用户存在两种操作：
第一种是，用户开团，保存在coco_groupon中，用户主动分享商品团购页面给朋友；
第二张是，用户参团，也保存在coco_groupon中。

只有开团人数符合团购规则条件，创建的订单才会有效，否则管理员需要退款取消当前团购。

### 2.7 优惠券设计

优惠券由coco_coupon表和coco_coupon_user表组成：
* coco_coupon表，是优惠券基本信息及使用规则。
* coco_coupon_user表，是用户优惠券领取和使用的记录。
...
这里需要指出的是，coco_coupon表和coco_coupon_user表都有status字段。

coco_coupon表的status字段，包含以后三种状态：
* 正常可用，
* 已过期，
* 已下架，

coco_coupon_user表的status字段，包含以后三种状态：
* 未使用，
* 已使用，
* 已过期，
* 已下架，
...
除了以上表，数据库还存在其他一些业务表，例如专题表coco_topic，
但是都很直观，不需要多讨论。
